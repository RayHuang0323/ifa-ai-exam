...
import { getDailyQuestionPool, getFullMockQuestionPool, getPracticeQuestionPool } from '../utils/questionEngine';
...
export default function CoachDashboard({ onReturnHome }: { onReturnHome: () => void }) {
  ...
  const eligibleVerifiedCount = fullMockQuestions.filter((question) => question.reviewStatus === 'verified').length;
  const eligibleSourceVerifiedCount = fullMockQuestions.filter((question) => question.reviewStatus === 'source_verified').length;
  const dailyQuestionPoolCount = getDailyQuestionPool().length;
  ...
  return (
    <main className="min-h-screen w-full bg-[#050508] text-slate-100"><div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      ...
      <section data-testid="question-bank-version" className="rounded-xl border border-indigo-800/70 bg-indigo-950/20 p-4 text-sm text-indigo-100"><p className="font-bold">題庫版本：{questionBankVersionLabel}</p><p className="mt-1">正式模擬題庫：{fullMockQuestions.length} 題（verified {eligibleVerifiedCount} + 教材證據題 {eligibleSourceVerifiedCount}）</p><p className="mt-1">Daily／Weekly 題池：{dailyQuestionPoolCount} 題 · 更新：{questionBankStats.updatedAt}</p><p className="mt-1">正式題第一輪：{formalFirstRound.appearedCount} / {formalFirstRound.totalCount} 題 · 練習題第一輪：{practiceFirstRound.appearedCount} / {practiceFirstRound.totalCount} 題</p><p className="mt-1">近 7 日重複率：Daily {getRecentRepeatRate('daily')}% · Weekly {getRecentRepeatRate('weekly')}% · 模擬考 {getRecentRepeatRate('mock')}%</p><p className="mt-1 text-xs text-indigo-200">目前可用題池：{dailyQuestionPoolCount} 題</p></section>
