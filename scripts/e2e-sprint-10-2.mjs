...
const UI = {
  ...
  required: ['模擬考中心', '最近學習紀錄', '學習路線圖', '使用中', '尚未開放', '建置中'],
};
...
const fail = async (page, message) => { await page.screenshot({ path: 'test-results/sprint-10-2-failed.png', fullPage: true }); console.error('URL:', page.url()); console.error((await body(page)).slice(0, 1000)); console.error('draft:', await page.evaluate(() => localStorage.getItem('ifa-week1-exam-draft-v1'))); throw new Error(message); };
async function verifyResume(page) { step('[Resume] start'); await page.goto(URL, { waitUntil: 'domcontentloaded', timeout: 15000 }); await page.evaluate(() => localStorage.clear()); await page.reload({ waitUntil: 'domcontentloaded', timeout: 15000 }); step('[Resume] enter'); await clickButton(page, UI.startExam); await clickButton(page, UI.confirmStart); await page.getByRole('radio').first().click({ timeout: 10000 }); step('[Resume] leave'); await clickButton(page, UI.leaveExam); await clickButton(page, UI.confirmLeave); step('[Resume] check'); if (!(await body(page)).includes(UI.resumeExam)) await fail(page, 'Resume button missing'); if (!(await page.evaluate(() => localStorage.getItem('ifa-week1-exam-draft-v1')))) await fail(page, 'Draft missing'); await clickButton(page, UI.resumeExam); }
...
async function main() { step('[Server] start'); server = spawn('cmd.exe', ['/c', 'npm.cmd run dev -- --host 127.0.0.1 --port 5173'], { stdio: 'ignore' }); step('[Server] wait'); await waitForServer(); step('[Browser] launch'); browser = await chromium.launch({ executablePath: CHROME, headless: true }); const page = await browser.newPage(); await verifyResume(page); await verifyTaskCount(page); await verifyChineseUi(page); console.log('VERIFY PASSED'); }
